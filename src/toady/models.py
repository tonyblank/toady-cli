...
class PullRequest:
    ...
    Attributes:
        ...
        review_thread_count: Number of review threads on this PR
    """
    ...
    def __post_init__(self) -> None:
        ...
        try:
            # Validate pr_id
            if not isinstance(self.pr_id, str):
                raise create_validation_error(
                    field_name="pr_id",
                    invalid_value=type(self.pr_id).__name__,
                    expected_format="non-empty string",
                    message="pr_id must be a string",
                )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PullRequest":
        ...
        return cls(
            ...
            updated_at=updated_at,
            url=data["url"],
            review_thread_count=data.get("review_thread_count", 0),
        )
    ...
    def _parse_datetime(date_str: str) -> datetime:
        ...
        try:
            ...
            raise create_validation_error(
                ...
                invalid_value=date_str,
                expected_format="ISO datetime string",
                message=f"Unable to parse datetime: {str(e)}",
            ) from e

    def __str__(self) -> str:
        """Return a human-readable string representation."""
        return (
            f"PullRequest(#{self.number}, title='{self.title}', "
            f"author={self.author}, draft={self.is_draft})"
        )
