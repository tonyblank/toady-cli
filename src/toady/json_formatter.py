...
class JSONFormatter(BaseFormatter):
    ...
    def __init__(self, options: Optional[FormatterOptions] = None, **kwargs: Any) -> None:
    ...
    def format_threads(self, threads: List[ReviewThread]) -> str:
        ...
        try:
            ...
            for thread in threads:
                try:
                    ...
                    raise FormatterError(
                        f"Failed to serialize thread {getattr(thread, 'thread_id', 'unknown')}: {str(e)}",
                        original_error=e
                    ) from e
            ...
            if isinstance(e, FormatterError):
                raise
            raise FormatterError(
                f"Failed to format threads as JSON: {str(e)}",
                original_error=e
            ) from e
        ...
        try:
            ...
            for comment in comments:
                try:
                    ...
                    raise FormatterError(
                        f"Failed to serialize comment {getattr(comment, 'comment_id', 'unknown')}: {str(e)}",
                        original_error=e
                    ) from e
            ...
            if isinstance(e, FormatterError):
                raise
            raise FormatterError(
                f"Failed to format comments as JSON: {str(e)}",
                original_error=e
            ) from e
    ...
    def format_object(self, obj: Any) -> str:
        ...
        try:
            ...
            raise FormatterError(
                f"Failed to format object as JSON: {str(e)}",
                original_error=e
            ) from e
    ...
    def format_array(self, items: List[Any]) -> str:
        ...
        try:
            ...
            if isinstance(e, FormatterError):
                raise
            raise FormatterError(
                f"Failed to format array as JSON: {str(e)}",
                original_error=e
            ) from e
    ...
    def format_primitive(self, value: Union[str, int, float, bool, None]) -> str:
        ...
        try:
            ...
            raise FormatterError(
                f"Failed to format primitive value as JSON: {str(e)}",
                original_error=e
            ) from e
    ...
    def format_error(self, error: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise FormatterError(
                f"Failed to format error as JSON: {str(e)}",
                original_error=e
            ) from e

    def format_success_message(self, message: str, details: Optional[Dict[str, Any]] = None) -> str:
    ...
    def format_warning_message(self, message: str, details: Optional[Dict[str, Any]] = None) -> str:
    ...
    def format_reply_result(self, reply_info: Dict[str, Any], verbose: bool = False) -> str:
        ...
        optional_fields = [
            'pr_number', 'pr_title', 'pr_url', 'thread_url', 
            'parent_comment_author', 'body_preview', 'review_id', 'comment_id'
        ]
