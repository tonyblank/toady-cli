...
class CIRunner:
    ...
    def run_command(self, cmd: List[str], description: str, 
                   timeout: int = 300, check_output: bool = False) -> Tuple[bool, str, float]:
    ...
    def generate_summary_report(self) -> None:
        ...
        if self.failed_checks == 0:
            ...
            print(f"{Colors.OKGREEN}✨ Pipeline completed successfully in {total_duration:.2f} seconds{Colors.ENDC}\n")
        else:
            print(f"{Colors.FAIL}{Colors.BOLD}💥 PIPELINE FAILED!{Colors.ENDC}")
            print(f"{Colors.FAIL}❌ {self.failed_checks} of {self.total_checks} checks failed{Colors.ENDC}\n")
        ...
        for check_name, result in self.check_results.items():
            ...
            print(f"   {status_icon} {Colors.BOLD}{check_name.title():.<20}{Colors.ENDC} "
        ...
        if "testing" in self.check_results and "coverage" in self.check_results["testing"]:
        ...
        for step_name, step_func in pipeline_steps:
            try:
                ...
                if success:
                    ...
                    if not success:
                        self.print_failure(f"Pipeline stopped due to {step_name.lower()} failure")
                        break
